#!/usr/bin/env python3
"""
Local orchestrator for one repository prerequisite.

Stage order:
1) Attach the repository bootstrap environment to the AWS bootstrap role.
2) Write bootstrap environment variables on the repository.
3) Write bootstrap GitHub App secrets on the repository.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare one repository created from the template.")
    ...
    browser_group.add_argument(
        ...
        dest="open_browser",
        action="store_true",
        help="Open browser for GitHub App installation/configuration checks (default)",
    )
    browser_group.add_argument(
        "--no-open-browser",
        dest="open_browser",
    ...
    parser.add_argument("--force-create-app", action="store_true", help="Force creating a new app if repo bootstrap cannot reuse one")
    parser.add_argument(
...
def main() -> int:
    ...
    print_step("Starting repository prerequisite.")

    git_context = discover_github_context_from_git(repo_root)
    if git_context:
    ...
    print_step(f"Resolved repository prerequisite context: org='{args.org}', repo='{args.repo}', region='{args.aws_region}'.")

    attach_role_script = base_dir / "aws" / "01-attach-bootstrap-role.py"
    variables_script = base_dir / "github" / "02-write-bootstrap-variables.py"
    secrets_script = base_dir / "github" / "03-write-bootstrap-app-secrets.py"

    attach_role_command = [
        sys.executable,
        str(attach_role_script),
        "--org",
        args.org,
        "--repo",
        args.repo,
    ...
    if args.aws_profile.strip():
        attach_role_command.extend(["--aws-profile", args.aws_profile.strip()])
        variables_command.extend(["--aws-profile", args.aws_profile.strip()])
    ...
    if args.app_name.strip():
        secrets_command.extend(["--app-name", args.app_name.strip()])
    if args.aws_profile.strip():
        secrets_command.extend(["--aws-profile", args.aws_profile.strip()])
    if args.homepage_url.strip():
        secrets_command.extend(["--homepage-url", args.homepage_url.strip()])
    if args.open_browser:
        secrets_command.append("--open-browser")
    else:
        secrets_command.append("--no-open-browser")
    if args.force_create_app:
        secrets_command.append("--force-create-app")
    if args.skip_team_repo_admin_grant:
        secrets_command.append("--skip-team-repo-admin-grant")

    print("== Step 1/3: AWS bootstrap role trust ==")
    run_command(attach_role_command, cwd=base_dir, description="Attaching repository to AWS bootstrap role...")

    print("\n== Step 2/3: GitHub bootstrap variables ==")
    run_command(variables_command, cwd=base_dir, description="Writing bootstrap environment variables...")

    print("\n== Step 3/3: GitHub bootstrap app secrets ==")
    run_command(secrets_command, cwd=base_dir, description="Writing bootstrap environment secrets...")

    print("\nRepository prerequisite finished.")
    return 0
